...
function init() {
  ...
  new GLTFLoader().load("/assets/3d/Model-2.glb", function (gltf) {
    const loadedModel = gltf.scene;
    const specificMesh = loadedModel.children[1]
    console.log(specificMesh);
...
function changeTexture(newUrl) {
  if (scene) {
    const textureLoader = new THREE.TextureLoader();
    const specificMesh = scene.getObjectByName("polySurface1");
    if (specificMesh) {
      textureLoader.load(
        "assets/3d/76_leather texture-seamless.jpg",
        (bumpMap) => {
