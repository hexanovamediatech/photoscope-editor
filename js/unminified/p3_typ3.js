...
function init() {
  ...
  new GLTFLoader().load("/assets/3d/p3_typ3.glb", function (gltf) {
    const loadedModel = gltf.scene;
    const specificMesh = loadedModel.children[0].children[3];
    // const specificMesh = scene.getObjectByName("Booklet_Outer_1");
    ...
    textureLoader.load(
      ...
      (bumpMap) => {
        textureLoader.load(url, (texture) => {
          ...
          texture.repeat.set(1.28, -1.28);
          texture.offset.set(.845, 1.15);
          texture.wrapS = texture.wrapT = THREE.RepeatWrapping;
        });
      }
    );

    loadedModel.position.set(-0.01, -0.12, 0);
...
function changeTexture(newUrl) {
  if (scene) {
    const textureLoader = new THREE.TextureLoader();
    const specificMesh = scene.getObjectByName("P3_typ3_Top");
    if (specificMesh) {
      textureLoader.load(url, (texture) => {
        const material = new THREE.MeshStandardMaterial({
          map: texture,
