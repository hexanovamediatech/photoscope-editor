...
function init() {
  ...
  camera.position.set(0, 0.01, 0.14);

  scene = new THREE.Scene();
  new GLTFLoader().load("/assets/3d/P9_type1.glb", function (gltf) {
    const loadedModel = gltf.scene;
    const specificMesh = loadedModel.children[0].children[1]
...
const specificMesh4 = loadedModel.children[0].children[0]
...
function changeTexture(newUrl ) {
  if (scene) {
    const textureLoader = new THREE.TextureLoader();
    const specificMesh = scene.getObjectByName("Ear_L2");
    console.log(specificMesh,'from here')
    if (specificMesh) {
      textureLoader.load(url, (texture) => {
        console.log("Texture second changed successfully:", texture);
        const material = new THREE.MeshStandardMaterial({
          map: texture,
          normalMap: texture,
          bumpMap: texture,
